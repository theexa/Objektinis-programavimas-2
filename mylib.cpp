...
void Strategija2(Container& studentai, Container& vargsiukai) {
    ...
    auto new_end = std::remove_if(studentai.begin(), studentai.end(),
        [&vargsiukai](const Studentas& s) {
            bool is_vargsiukas = (s.gal < 5.0);
            if (is_vargsiukas) {
                vargsiukai.push_back(s);
            }
            return is_vargsiukas;
        });
...
void Strategija3(Container& studentai, Container& vargsiukai, Container& kietiakai) {
    ...
    Container studentai_kopija3 = studentai;
    Container studentai_kopija4 = studentai;
    ...
    if (laikas1 < laikas2) {
        ...
        OptimizuotaStrategija1(studentai_kopija3, vargsiukai, kietiakai);
        IrasytiIFaila("RezA.txt", vargsiukai);
        IrasytiIFaila("RezB.txt", kietiakai);
    } else {
        cout << "2 strategija yra greitesnė" << endl;
        OptimizuotaStrategija2(studentai_kopija4, vargsiukai);
        kietiakai = std::move(studentai_kopija2);
        IrasytiIFaila("RezA.txt", vargsiukai);
        IrasytiIFaila("RezB.txt", kietiakai);  // Fixed: write kietiakai, not empty container
    }
    cout << "Rezultatas: Vargsiukų: " << vargsiukai.size() << ", Kietiakų: " << kietiakai.size() << endl;
    studentai_kopija1.clear();
    studentai_kopija2.clear();
    studentai_kopija3.clear();
    studentai_kopija4.clear();
}
...
double VykdytiStrategija(Container& studentai, int strategija) {
    ...
    // Create a copy for each test to preserve original data
    Container studentai_kopija = studentai;
    
    switch(strategija) {
        case 1: {
            Container vargsiukai, kietiakai;
            laikas = MatuotiLaika("Strategija 1 ", [&]() {
                Strategija1(studentai_kopija, vargsiukai, kietiakai);
            });
        ...
        case 2: {
            ...
            laikas = MatuotiLaika("Strategija 2 ", [&]() {
                Strategija2(studentai_kopija, vargsiukai);
            });
            cout << "Vargsiukų: " << vargsiukai.size() << ", Liko studentų: " << studentai_kopija.size() << endl;
            break;
        }
        case 3: {
            Container vargsiukai, kietiakai;
            laikas = MatuotiLaika("Strategija 3 ", [&]() {
                Strategija3(studentai_kopija, vargsiukai, kietiakai);
            });
            break;
        }
        default:
